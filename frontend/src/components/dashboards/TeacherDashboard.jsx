...
import { BookOpen, Calendar, Clock, CheckCircle, Users, Building2, Award, ClipboardCheck, ArrowUpRight } from 'lucide-react';
...
export const TeacherDashboard = () => {
  ...
  const schedule = [
    { time: '09:30 AM - 10:45 AM', subject: 'Advanced Operating Systems', room: 'AK-302 (Kalam Block 3rd Floor)', batch: 'CSE-4th Sem (Sec A)', attendance: '48/52 Present' },
    { time: '11:15 AM - 01:00 PM', subject: 'Distributed Cloud Architecture Lab', room: 'Kalam AI Lab 402', batch: 'CSE-6th Sem (Sec B)', attendance: 'Scheduled' },
    { time: '02:30 PM - 03:45 PM', subject: 'Design & Analysis of Algorithms', room: 'AK-204 (Kalam Block 2nd Floor)', batch: 'CSE-4th Sem (Sec C)', attendance: 'Scheduled' },
  ];
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center gap-2">
              ...
              <span>Faculty Cabin: <strong>Kalam Block - 2nd Floor (Cabin 12)</strong></span>
            </p>
          </div>

          <div
            className={`px-4 py-3 rounded-2xl border text-xs font-semibold ${
              isAmoled ? 'bg-neutral-900/90 border-white/10 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-700'
            }`}
          >
            <span className="text-slate-400 block text-[10px] uppercase font-bold">Faculty Identifier</span>
            <span className="text-sm font-extrabold text-brand-cyan">ID: {user?.identifier || 'T101'}</span>
          </div>
        </div>
      </div>
